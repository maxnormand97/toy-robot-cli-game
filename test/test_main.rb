...
class ToyRobotCLITest < Minitest::Test
  def test_run_prints_welcome_and_help_on_startup
    output = run_commands('EXIT')

    assert_output_includes(output, 'Welcome to the Toy Robot CLI Game!', 'Commands:', 'PLACE X,Y,O')
  end

  def test_help_command_prints_usage_information
    output = run_commands('HELP', 'EXIT')
  ...
  def test_report_before_placement_prints_error_message
    output = run_commands('REPORT', 'EXIT')
  ...
  def test_place_outside_grid_prints_bounds_error
    output = run_commands('PLACE 6,6,N', 'EXIT')
  ...
  def test_unknown_command_prints_guidance
    output = run_commands('DANCE', 'EXIT')
  ...
  def run_commands(*commands)
    run_cli("#{commands.join("\n")}\n")
  end

  def assert_output_includes(output, *expected_fragments)
    expected_fragments.each do |fragment|
      assert_includes output, fragment
    end
  end
